...
import { getRunningQueriesThunk, getSearchAnimals, useGetSearchAnimalsQuery } from "@/store/api/animals";
...
export default function Home(
  ...
  const name = router.query.name as string|undefined;
   const result = useGetSearchAnimalsQuery({search:name?name:"", page:page?page:""},
  );
  const detailsResult = useGetSearchAnimalsQuery({search:details?details:"", page:page?page:""},
  );
  const { data } = result;
...
export const getServerSideProps = wrapper.getServerSideProps( (store) => async (context) => {
  ...
  const { details } = context.query as { details: string | undefined };
    ...
    if(details){
      store.dispatch(getSearchAnimals.initiate({search:details, page:firstPage}));
    }else{
      store.dispatch(getSearchAnimals.initiate({search:name?name:"", page:page?page:firstPage}));
    }
